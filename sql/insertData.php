...
try {
    ...
    foreach ($data as $artistName => $albums) {
        ...
        foreach ($albums as $album) {
            ...
            $stmtProduct = $db->prepare("INSERT INTO products (title, product_description, created) VALUES (:albumName, :description, NOW())");
...
$sql = "INSERT INTO users (first_name, last_name, email, user_password, creation_date, role_id) VALUES ('Noga', 'Vigdor', 'admin@cdharmony.dk','".$Hpass."','".date('Y-m-d H:i:s')."','1')";// Prepare the statement
$stmt = $db->prepare($sql);
// Execute the statement
$stmt->execute();
...
/*  
    function insertArticle($db, $title, $content, $publish_date, $update_date, $user_id)
    {
        ...
        $stmt = $db->prepare('INSERT INTO articles (title, content, publish_date, updated_date, user_id) VALUES (:title, :content, :publish_date, :update_date, :user_id)');
        $stmt->bindParam(':title', $title, PDO::PARAM_STR);
        $stmt->bindParam(':content', $content, PDO::PARAM_STR);
        $stmt->bindParam(':publish_date', $publish_date, PDO::PARAM_STR);
        $stmt->bindParam(':updated_date', $update_date, PDO::PARAM_STR);
...
$description = "High-quality CD player from Sony.";

$stmt = $db->prepare("INSERT INTO products (title, product_description, created) VALUES (:title, :product_description, NOW())");
$stmt->bindParam(':title', $title, PDO::PARAM_STR);
$stmt->bindParam(':product_description', $description, PDO::PARAM_STR);
$stmt->execute();

// Get the last inserted product_id
$productIdForSonyCDPlayer = $db->lastInsertId();
$brand = "Sony";
$model = "CDP-101";
$stmt = $db->prepare("INSERT INTO electronic_devices (brand, model, product_id) VALUES (:brand, :model, :productId)");
$stmt->bindParam(':brand', $brand, PDO::PARAM_STR);
$stmt->bindParam(':model', $model, PDO::PARAM_STR);
$stmt->bindParam(':productId', $productIdForSonyCDPlayer, PDO::PARAM_INT);
$stmt->execute();
